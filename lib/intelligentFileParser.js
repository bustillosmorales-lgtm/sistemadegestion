...
class IntelligentFileParser {
    constructor() {
        ...
        this.columnMappings = {
            ...
            container_number: ['container_number', 'contenedor', 'container', 'numero_contenedor'],
            proveedor: ['proveedor', 'supplier', 'vendor', 'provider'],
            status_compra: ['status_compra', 'purchase_status', 'estado', 'status'],
            
            // Categorización
            categoria: ['categoria', 'category', 'tipo', 'type', 'classification'],
    ...
    validateData(parsedData, expectedType = null) {
        ...
        data.forEach((row, index) => {
            ...
            if (expectedType === 'ventas') {
                if (!row.numero_venta) {
                    warnings.push(`Fila ${rowNum}: Número de venta no especificado`);
                }
            }
            
            if (expectedType === 'compras') {
                if (!row.numero_compra) {
                    warnings.push(`Fila ${rowNum}: Número de compra no especificado`);
                }
            }
            
            // Validación de fechas
            const dateFields = ['fecha_venta', 'fecha_compra', 'fecha_llegada_estimada'];
            dateFields.forEach(field => {
                if (row[field] && row[field] !== '' && row[field] === row._original[field]) {
                    warnings.push(`Fila ${rowNum}: Formato de fecha cuestionable en ${field}: ${row[field]}`);
                }
            });
        });
