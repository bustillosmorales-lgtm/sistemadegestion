...
const RequestQuoteForm = ({ data, setData, product }) => {
    // Enhanced data loading with fallback API for missing breakdown
    const { data: enhancedData, error: enhancedError } = useSWR(
        !product.breakdown ? `/api/product-quote-info?sku=${product.sku}` : null, 
        fetcher
    );
    
    // Use enhanced data if breakdown is missing
    const effectiveProduct = !product.breakdown && enhancedData?.results?.[0] 
        ? { ...product, ...enhancedData.results[0] }
        : product;
    
    // Loading state for missing breakdown data
    const isLoadingBreakdown = !product.breakdown && !enhancedData && !enhancedError;
    
    const diasDisponibles = effectiveProduct.breakdown?.diasCoberturaLlegada;
    const esUrgente = diasDisponibles !== undefined && diasDisponibles < 0;
    const ventaDiariaDetails = effectiveProduct.breakdown?.ventaDiariaDetails;
    ...
    if (enhancedError) {
        return (
            <div className="text-center py-4 text-red-600">
                <p>Error cargando datos del producto. Por favor intente nuevamente.</p>
            </div>
        );
    }
    ...
    <>
        ...
        <div className="bg-gray-50 p-3 rounded-lg space-y-2 text-sm mb-4">
            ...
            {effectiveProduct.breakdown ? (
                <>
                    <div className="flex justify-between"><span>Stock Objetivo:</span><span className="font-mono">{effectiveProduct.breakdown.stockObjetivo} un.</span></div>
                    <hr/>
                    <div className="flex justify-between"><span>(+) Stock Actual:</span><span className="font-mono">{effectiveProduct.breakdown.stockActual} un.</span></div>
                    <div className="flex justify-between"><span>(+) Stock en Tránsito que llega:</span><span className="font-mono">{effectiveProduct.breakdown.stockEnTransitoQueLlega} un.</span></div>
                    <div className="flex justify-between"><span>(-) Consumo Proyectado:</span><span className="font-mono">-{effectiveProduct.breakdown.consumoDuranteLeadTime} un.</span></div>
                    <div className="flex justify-between border-t pt-1 mt-1"><span>(=) Stock Proyectado a la Llegada:</span><span className="font-mono">{effectiveProduct.breakdown.stockFinalProyectado} un.</span></div>
                    <hr/>
                    <div className="flex justify-between font-bold text-base mt-1"><span>Cantidad a Reponer Sugerida:</span><span className="font-mono text-blue-600">{effectiveProduct.cantidadSugerida} un.</span></div>
                </>
        ...
        <FormField label="Link de Referencia" name="link" value={effectiveProduct.link || 'N/A'} readOnly />
...
const ApprovePurchaseForm = ({ product, analysisDetails, onApprove, onReject, onClose }) => {
    ...
    console.log('🔍 ApprovePurchaseForm Debug:', {
        productSku: product.sku,
        analysisDetails,
        productAnalysisDetails: product.analysis_details
    });

    // Cargar detalles completos del producto si analysis_details no está disponible
    const { data: productDetails, error: productError } = useSWR(
        !analysisDetails?.sellingPrice ? `/api/products?sku=${product.sku}` : null,
        fetcher
    );

    // Usar datos del productDetails si están disponibles para tener información completa
    const effectiveProduct = productDetails || product;

    // El precio de venta ya debe estar guardado del paso anterior "Analizar"
    const precioVenta = analysisDetails?.sellingPrice || productDetails?.analysis_details?.sellingPrice;
    ...
    console.log('🔍 Product data for analysis:', {
        productSku: product.sku,
        productCbm: effectiveProduct.cbm,
        productCostoFobRmb: effectiveProduct.costo_fob_rmb,
        precioVenta,
        precioVentaEditable,
        analysisUrl: precioVentaEditable ? `/api/analysis?sku=${product.sku}&precioVenta=${precioVentaEditable}` : null,
        hasProductDetails: !!productDetails,
        effectiveProductKeys: Object.keys(effectiveProduct)
    });
    ...
    // SKU editing for auto-generated SKUs and new products
    const isAutoGeneratedSku = product.sku && product.sku.match(/^SKU-\d{6}$/);
    const isNewProduct = product.isNewProduct || 
                        (product.request_details?.createdFromForm) ||
                        (product.sku && (product.sku.includes('TEMP-') || product.sku.includes('NEW-')));
    const needsSkuChange = isAutoGeneratedSku || isNewProduct;
    
    // Generate suggested internal SKU for new products
    ...
    const [editableSku, setEditableSku] = useState(() => {
        if (isNewProduct && !product.hasInternalSku) {
            return generateInternalSku();
        }
        return product.sku || '';
    });
    ...
    const suggestedQuantity = effectiveProduct.cantidadSugerida || 0;
    ...
    // Actualizar precioVentaEditable cuando se cargan los datos
    useEffect(() => {
        if (precioVenta && !precioVentaEditable) {
            setPrecioVentaEditable(precioVenta);
        }
    }, [precioVenta, precioVentaEditable]);

    if (error || productError) {
        console.log('❌ Errors in ApprovePurchaseForm:', { error, productError });
        return (
            <div className="space-y-4">
                <div className="text-red-500">
                    Error al calcular análisis.
                    {error && <div className="text-xs mt-1">Error análisis: {error.message}</div>}
                    {productError && <div className="text-xs mt-1">Error producto: {productError.message}</div>}
                </div>
    ...
    // Mostrar estado de carga si estamos obteniendo los detalles del producto
    if (!analysisDetails?.sellingPrice && !productDetails && !productError) {
        return (
            <div className="space-y-4">
                <div className="text-center py-4">
    ...
    const calculateTargetPrice = () => {
        ...
        const costoLogisticoCLP = analysis.costoFinalBodega - (sellingPrice * config.usdToClp *
            ((effectiveProduct.costo_fob_rmb * config.rmbToUsd * (1 + config.costosVariablesPct.comisionChina)) +
            ((effectiveProduct.costo_fob_rmb * config.rmbToUsd * (1 + config.costosVariablesPct.comisionChina)) * config.costosVariablesPct.seguroContenedor) +
            (config.costosFijosUSD.fleteMaritimo * effectiveProduct.cbm / config.containerCBM)) *
        ...
        const fleteUSD = config.costosFijosUSD.fleteMaritimo * effectiveProduct.cbm / config.containerCBM;
    ...
    const createPayload = (approved) => ({
        ...
        newSku: needsSkuChange && editableSku !== product.sku ? editableSku : null,
        manualSkuChange: editableSku !== product.sku && !editableSku.startsWith('INT-'),
    ...
    return (
        <div className="space-y-4 text-sm">
            ...
            {/* SKU Editing for auto-generated SKUs and new products */}
            {needsSkuChange && (
                <div className={`border rounded-lg p-4 ${isNewProduct ? 'bg-blue-50 border-blue-200' : 'bg-yellow-50 border-yellow-200'}`}>
                    <h4 className={`font-semibold mb-3 ${isNewProduct ? 'text-blue-800' : 'text-yellow-800'}`}>
                        {isNewProduct ? '🆕 Producto Nuevo - Asignar SKU Interno' : '🏷️ Código SKU del Producto'}
                    </h4>
                    {!skuWarningDismissed && (
                        <div className={`border rounded-lg p-3 mb-4 ${isNewProduct ? 'bg-blue-100 border-blue-300' : 'bg-yellow-100 border-yellow-300'}`}>
                            <div className="flex items-start">
                                <span className={`mr-2 ${isNewProduct ? 'text-blue-600' : 'text-yellow-600'}`}>
                                    {isNewProduct ? '🆕' : '💡'}
                                </span>
                                <div className="flex-1">
                                    <p className={`text-sm font-medium mb-1 ${isNewProduct ? 'text-blue-800' : 'text-yellow-800'}`}>
                                        {isNewProduct ? 'Producto Nuevo Detectado' : 'SKU Auto-generado Detectado'}
                                    </p>
                                    <p className={`text-xs mb-2 ${isNewProduct ? 'text-blue-700' : 'text-yellow-700'}`}>
                                        {isNewProduct 
                                            ? 'Este es un producto nuevo que necesita un SKU interno para futuras reposiciones. Se ha pre-generado un código interno sugerido que puede modificar.'
                                    ...
                                    <button 
                                        ...
                                        className={`text-xs underline hover:opacity-75 ${isNewProduct ? 'text-blue-600' : 'text-yellow-600'}`}
                                    >
                                        Entendido, continuar
                                    </button>
                                </div>
                    ...
                    <div className="flex gap-2">
                        <div className="flex-1">
                            <FormField 
                                label={isNewProduct ? "Código SKU Interno" : "Código SKU"}
                                name="editableSku" 
                                type="text" 
                                value={editableSku} 
                        ...
                        {isNewProduct && (
                            <div className="flex items-end">
                                <button
                                    type="button"
                                    onClick={() => setEditableSku(generateInternalSku())}
                                    className="px-3 py-2 bg-blue-600 text-white text-xs rounded hover:bg-blue-700"
                                    title="Generar nuevo SKU interno"
                                >
                                    🔄 Nuevo SKU
                                </button>
                            </div>
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                        {isNewProduct 
                            ? 'SKU interno sugerido formato: INT-YYYYMMDD-XXX. Debe ser único en el sistema.'
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between"><span>CBM por Unidad:</span> <span className="font-mono">{(effectiveProduct.cbm || 0).toFixed(4)} m³</span></div>
                            <div className="flex justify-between"><span>Precio FOB (RMB):</span> <span className="font-mono">¥{(effectiveProduct.costo_fob_rmb || 0).toFixed(2)}</span></div>
                            <div className="flex justify-between"><span>Tasa RMB → USD:</span> <span className="font-mono">{config.rmbToUsd || 0.14}</span></div>
                            <div className="flex justify-between border-t pt-2"><span className="font-semibold">Precio FOB (USD):</span> <span className="font-mono font-semibold">${((effectiveProduct.costo_fob_rmb || 0) * (config.rmbToUsd || 0.14)).toFixed(2)}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2">
                            {(() => {
                                ...
                                const costoFobUSD = (effectiveProduct.costo_fob_rmb || 0) * (config.rmbToUsd || 0);
                                ...
                                const fletePorProductoUSD = ((config.costosFijosUSD?.fleteMaritimo || 0) / (config.containerCBM || 1)) * (effectiveProduct.cbm || 0);
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2">
                            {(() => {
                                ...
                                const costoLogisticoProductoUSD = costoLogisticoPorCBM_USD * (effectiveProduct.cbm || 0);
...
const PurchaseConfirmationForm = ({ data, setData, product }) => {
    // Cargar detalles completos del producto si approval_details no está disponible
    const { data: productDetails, error: productError } = useSWR(
        !product.approval_details?.purchaseQuantity ? `/api/products?sku=${product.sku}` : null,
        fetcher
    );

    const effectiveProduct = productDetails || product;
    const approvedQuantity = effectiveProduct.approval_details?.purchaseQuantity || 0;

    // Mostrar estado de carga si estamos obteniendo los detalles del producto
    if (!product.approval_details?.purchaseQuantity && !productDetails && !productError) {
        return (
            <div className="text-center py-4">
                <p>Cargando detalles de la aprobación...</p>
            </div>
        );
    }
...
const ManufacturingForm = ({ data, setData, product }) => {
    // Cargar detalles completos del producto si purchase_details no está disponible
    const { data: productDetails, error: productError } = useSWR(
        !product.purchase_details?.confirmedQuantity && !product.approval_details?.purchaseQuantity ? `/api/products?sku=${product.sku}` : null,
        fetcher
    );

    const effectiveProduct = productDetails || product;
    const purchaseQuantity = effectiveProduct.purchase_details?.confirmedQuantity || effectiveProduct.approval_details?.purchaseQuantity || 0;
...
const ShippingForm = ({ data, setData, product }) => {
    ...
    // Cargar detalles completos del producto si manufacturing_details no está disponible
    const { data: productDetails, error: productError } = useSWR(
        !product.manufacturing_details?.manufacturedQuantity && !product.purchase_details?.confirmedQuantity && !product.approval_details?.purchaseQuantity ? `/api/products?sku=${product.sku}` : null,
        fetcher
    );

    const effectiveProduct = productDetails || product;
    const manufacturedQuantity = effectiveProduct.manufacturing_details?.manufacturedQuantity || effectiveProduct.purchase_details?.confirmedQuantity || effectiveProduct.approval_details?.purchaseQuantity || 0;
