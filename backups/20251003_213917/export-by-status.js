...
function calcularCantidadSugerida(product, transitMap, config) {
  const ventaDiaria = product.venta_diaria || 0;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3️⃣ Obtener detalles adicionales necesarios
    const skus = products.map(p => useCache ? p.sku : p.sku);
    ...
    // 4️⃣ Obtener fechas de análisis de ventas
    const { data: ventasDiariasFechas } = await supabase
      .from('sku_venta_diaria_mv')
      .select('sku, fecha_inicio, fecha_fin, dias_periodo, total_vendido')
      .in('sku', skus);

    const fechasMap = Object.fromEntries(
      (ventasDiariasFechas || []).map(v => [v.sku, {
        fechaInicio: v.fecha_inicio,
        fechaFin: v.fecha_fin,
        diasPeriodo: v.dias_periodo,
        unidadesVendidas: v.total_vendido
      }])
    );
...
function formatByAction(action, products, detailsMap, transitMap, config, useCache, fechasMap) {
  const formatters = {
    ...
    view: (products) => products.map(p => {
      ...
      const fechasAnalisis = fechasMap[p.sku] || null;
      ...
      return {
        ...
        'Periodo Análisis': fechasAnalisis ? `${fechasAnalisis.fechaInicio || ''} a ${fechasAnalisis.fechaFin || ''}` : 'N/A',
        'Días del Periodo': fechasAnalisis?.diasPeriodo || 'N/A',
        'Unidades Vendidas': fechasAnalisis?.unidadesVendidas || 'N/A',
        'Status': p.status
      };
    }),
    ...
    request_quote: (products) => products.map(p => {
      const stockActual = useCache ? p.stock_actual : p.stock_actual || 0;
      const ventaDiaria = useCache ? p.venta_diaria : (p.venta_diaria || 0);
      const enTransito = useCache ? p.en_transito : (transitMap[p.sku] || 0);

      // Calcular cantidadSugerida usando la fórmula correcta
      const calculos = useCache ?
        {
          cantidadSugerida: p.cantidad_sugerida,
          stockObjetivo: p.stock_objetivo,
          stockProyectadoLlegada: p.stock_proyectado_llegada,
          consumoDuranteLeadTime: p.consumo_durante_lead_time
        } :
      ...
      return {
        ...
        'Fecha Inicial Análisis': fechasAnalisis?.fechaInicio || 'N/A',
        'Fecha Final Análisis': fechasAnalisis?.fechaFin || 'N/A',
        'Unidades Vendidas Periodo': fechasAnalisis?.unidadesVendidas || 'N/A',
        'Días del Periodo': fechasAnalisis?.diasPeriodo || 'N/A',
        'Venta Diaria Promedio': ventaDiaria?.toFixed(2) || '0.00',
