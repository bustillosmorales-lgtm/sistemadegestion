...
class IntelligentFileParser {
    constructor() {
        ...
        this.columnMappings = {
            ...
            fecha_llegada_real: ['fecha_llegada_real', 'actual_arrival', 'arrival_date', 'fecha_real', 'actual_arrival_date', 'llegada_real'],
            ...
            fecha_efectiva_llegada: ['fecha_efectiva_llegada', 'fecha_efectiva', 'effective_arrival', 'fecha_bodega', 'arrival_to_warehouse', 'warehouse_arrival'],
            ...
            // Campos específicos de packs
            pack_sku: ['pack_sku', 'IDPack', 'id_pack', 'sku_pack', 'pack_id', 'pack'],
            producto_sku: ['producto_sku', 'IDProducto', 'id_producto', 'product_sku', 'item_sku', 'sku_producto'],
        };
    }
    ...
    analyzeDataFormat(data) {
        ...
        const hasPacks = sample.some(row => (row.pack_sku || row.IDPack) && (row.producto_sku || row.IDProducto));
        ...
        if (hasPacks) analysis.probableType = 'packs';
    ...
    validateData(parsedData, expectedType = null) {
        ...
        data.forEach((row, index) => {
            ...
            if (expectedType === 'packs') {
                // Validar campos obligatorios para packs
                if (!row.pack_sku && !row.IDPack) {
                    errors.push(`Fila ${rowNum}: Pack SKU (IDPack) es requerido`);
                }

                if (!row.producto_sku && !row.IDProducto) {
                    errors.push(`Fila ${rowNum}: Producto SKU (IDProducto) es requerido`);
                }
                ...
                // Advertencias
                if ((row.pack_sku || row.IDPack) && (row.producto_sku || row.IDProducto)) {
                    if (!this.looksLikeSku(row.pack_sku || row.IDPack)) {
                        warnings.push(`Fila ${rowNum}: El Pack SKU "${row.pack_sku || row.IDPack}" podría no tener un formato válido`);
                    }
                }
            }
