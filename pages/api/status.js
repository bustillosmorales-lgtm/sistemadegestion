    ...
    if (isValidTransition) {
        ...
        switch (product.status) {
            ...
            case 'ANALYZING':
                ...
                // Detectar productos nuevos y generar SKU interno automáticamente al aprobar
                if (nextStatus === 'PURCHASE_APPROVED' && payload.approved) {
                    const isNewProduct = product.isNewProduct || 
                                       (product.request_details?.createdFromForm) ||
                                       (sku.includes('TEMP-') || sku.includes('NEW-'));
                    
                    if (isNewProduct && !payload.newSku) {
                        // Generar SKU interno automáticamente
                        ...
                        const suggestedSku = `INT-${year}${month}${day}-${random}`;
                        
                        // Verificar que no exista este SKU
                        ...
                        if (!existingSku) {
                            payload.newSku = suggestedSku;
                            console.log('🏷️ SKU interno generado automáticamente:', `${sku} → ${suggestedSku}`);
                        }
                    }
                }
                
                // Handle SKU change for auto-generated SKUs or manual changes
                if (payload.newSku && payload.newSku !== sku) {
                    ...
                    updateData.approval_details.skuChanged = {
                        ...
                        to: payload.newSku,
                        timestamp,
                        isNewProduct: product.isNewProduct || (product.request_details?.createdFromForm) || false,
                        autoGenerated: !payload.manualSkuChange || false
                    };
                    
                    // Marcar como producto que ya tiene SKU interno
                    updateData.isNewProduct = false;
                    updateData.hasInternalSku = true;
